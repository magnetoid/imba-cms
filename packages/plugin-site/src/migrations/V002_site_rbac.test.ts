...
import v001 from './V001_site.sql?raw'
import raw from './V002_site_rbac.sql?raw'
...
describe('site V002 rbac migration', () => {
  it('retires the is_admin()-gated write policy', () => {
    // V001 gated writes on is_admin(), which resolves only super_admin — while
    // the admin UI showed the editor to anyone with site.write. Saves from a
    // content_admin or editor were silently swallowed by RLS.
    expect(v001).toMatch(/create policy site_entries_write[\s\S]*is_admin\(\)/i)
    expect(sql).toMatch(/drop policy if exists site_entries_write on site_entries/i)
  ...
  it('closes the draft leak to anonymous callers', () => {
    ...
    expect(v001).toMatch(/create policy site_entries_read[\s\S]*?using \(true\)/i)
    expect(sql).toMatch(/drop policy if exists site_entries_read on site_entries/i)
    expect(sql).toMatch(
      /create policy site_entries_public_read[\s\S]*?using \(status = 'published'\)/i,
    )
  })

  it('gives staff a separate draft-visible read policy', () => {
    expect(sql).toMatch(
      /create policy site_entries_staff_read[\s\S]*?has_capability\('site\.read'\)/i,
    )
  })

  it.each(['insert', 'update', 'delete'])('gates %s on a capability', (operation) => {
    const policy = new RegExp(
      `create policy site_entries_${operation}[\\s\\S]*?has_capability\\('site\\.(write|publish)'\\)`,
      'i',
    )
  ...
  it('lets publish permit an update, so a reviewer can act without write', () => {
    const update = /create policy site_entries_update([\s\S]*?);/i.exec(sql)
    expect(update).not.toBeNull()
    expect(update![1]).toMatch(/site\.publish/)
  })
